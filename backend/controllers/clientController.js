...
const getClientData = (req, res) => {
    db.get("SELECT id, clientId, username, role, wallet, area, contact, isActive, commissionRates, prizeRates FROM clients WHERE id = ?", [req.user.id], (err, row) => {
...
const getClientBets = (req, res) => {
    db.all("SELECT * FROM bets WHERE clientId = ? ORDER BY createdAt DESC", [req.user.id], (err, rows) => {
        if (err) return res.status(500).json({ message: err.message });
        res.json(rows);
    });
};
...
const getClientTransactions = (req, res) => {
     db.all("SELECT * FROM transactions WHERE clientId = ? ORDER BY createdAt DESC", [req.user.id], (err, rows) => {
        if (err) return res.status(500).json({ message: err.message });
        res.json(rows);
    });
};
    ...
    db.get("SELECT wallet FROM clients WHERE id = ?", [clientId], (err, client) => {
        ...
        db.serialize(() => {
            ...
            betsToPlace.forEach(bet => {
                betStmt.run(`bet-${uuidv4()}`, clientId, bet.drawId, bet.gameType, bet.number, bet.stake, new Date().toISOString(), bet.condition);
            });
